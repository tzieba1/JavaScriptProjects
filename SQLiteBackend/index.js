      ...
      db.run("INSERT INTO Inventory(animal,description,age,price) VALUES (?,?,?,?)",
        ...
        function (err, results) {
          if (err) {
            ...
            console.log(err);
            res.json({
              "error": "Could not insert animal"
            });
          } else {
            console.log(results);
            res.json({
              "status": "Add animal successful"
            });
          }
      ...
      db.run("DELETE FROM Inventory WHERE rowid=?",
        ...
        function (err, results) {
          if (err) {
            ...
            console.log(results);
            res.json({
              "status": "Delete animal successful"
            });
          }
      ...
      db.run("UPDATE Inventory SET animal=(?), description=(?), " +
        ...
        function (err, results) {
          if (err) {
            ...
            console.log(results);
            res.json({
              "status": "Update animal successful"
            });
          }
      ...
      db.all("SELECT rowid as id, animal, description, age, price FROM Inventory " +
        ...
        function (err, results) {
          if (err) {
            console.log(err);
            res.json({
              "error": "Could not search inventory"
            });
          } else {
            // send debug info to console
            console.log(JSON.stringify(results));
